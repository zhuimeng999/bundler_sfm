...
def bundler(image_list=None, options_file=None, shell=False, *args, **kwargs):
    ...
    if type(image_list) == dict:
        with tempfile.NamedTemporaryFile(delete=False, mode='w') as fp:
            for image,value in sorted(image_list.items(), key=lambda it: it[0]):
                if value == None: fp.write(' '.join([image, '0', str(2400), '\n']))
